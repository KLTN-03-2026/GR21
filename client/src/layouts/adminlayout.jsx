...
const AdminLayout = () => {
    ...
    // ✅ LẤY DỮ LIỆU USER ĐÃ JOIN TỪ BACKEND
    const userData = JSON.parse(localStorage.getItem('user') || '{}');

    const menuItems = [
        { id: 'dashboard', label: 'Bảng điều khiển', icon: '📊', path: '/admin/dashboard' },
        { id: 'employees', label: 'Quản lý nhân viên', icon: '👥', path: '/admin/employees' },
        { id: 'departments', label: 'Quản lý phòng ban', icon: '🏢', path: '/admin/phong-ban' },
        { id: 'attendance', label: 'Quản lý chấm công', icon: '⏰', path: '/admin/cham-cong' },
        { id: 'leaves', label: 'Quản lý nghỉ phép', icon: '📅', path: '/admin/leaves' },
    ...
    // Tìm label hiện tại dựa trên URL để hiển thị lên Header
        ...
        <div className="flex min-h-screen bg-slate-50 font-sans text-slate-900">
            ...
            <aside className="w-80 bg-slate-900 text-white p-8 hidden lg:flex flex-col sticky top-0 h-screen shadow-2xl">
                <div className="flex items-center gap-4 mb-12 px-2">
                    <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center font-black text-xl shadow-lg shadow-indigo-500/20 text-white animate-pulse">21</div>
                    <div className="flex flex-col text-white">
                        <span className="font-black text-xl tracking-tighter uppercase leading-none italic">Admin</span>
