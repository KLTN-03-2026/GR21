...
const PhongBan = () => {
    ...
    return (
        <div className="w-full animate-in fade-in duration-700 relative">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                ...
                                <tr key={dept.id} className="hover:bg-white group transition-all">
                                    ...
                                    {/* --- CỘT NGƯỜI ĐỨNG ĐẦU (ĐÃ FIX HIỆN DATABASE DƯỚI LÊN) --- */}
                                    <td className="p-6">
                                        {dept.manager_name ? (
                                            <div className="flex flex-col gap-1">
                                                ...
                                                {/* Hiện Chức vụ từ Database lên luôn */}
                                    ...
                                    <td className="p-6 text-center">
                                        <div className="flex justify-center gap-3">
                                            <button className="bg-slate-100 hover:bg-indigo-100 text-indigo-600 p-3 rounded-xl transition-all active:scale-90">✏️</button>
                                            <button className="bg-slate-100 hover:bg-rose-100 text-rose-600 p-3 rounded-xl transition-all active:scale-90">🗑️</button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
            ...
            {showAddModal && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 animate-in fade-in zoom-in duration-200">
                    <div className="bg-white w-full max-w-lg rounded-[3rem] p-10 shadow-2xl border border-white">
                        <h3 className="text-2xl font-black uppercase italic text-slate-800 mb-8 tracking-tighter">➕ Tạo đơn vị mới</h3>
            ...
            {/* MODAL XEM DANH SÁCH NHÂN VIÊN THEO PHÒNG */}
            {showModal && (
                                    ...
                                    {deptEmployees.map(emp => (
                                        <div key={emp.id} className="flex items-center justify-between p-6 bg-white rounded-[2rem] shadow-sm border border-slate-100 hover:scale-[1.02] transition-all group/item shadow-inner">
                                            <div className="flex items-center gap-4">
                                                <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-white font-black text-lg group-hover/item:bg-indigo-600 transition-colors">
