        ...
        <div className="p-8 bg-slate-50 min-h-screen">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {leaves.map((item) => (
                    <div key={item.id} className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-100 hover:shadow-2xl transition-all duration-300 relative overflow-hidden group">
                        ...
                        {/* Employee Info */}
                        <div className="mb-6">
                            <h3 className="text-xl font-extrabold text-slate-800 uppercase">{item.full_name}</h3>
                        ...
                        {/* Action Buttons (Chỉ hiện khi đơn đang chờ) */}
                        {item.status === 'pending' ? (
                            ...
                            <div className="text-center text-[10px] font-bold text-slate-300 uppercase py-2 tracking-widest border-t border-slate-50">
            ...
            {leaves.length === 0 && (
                <div className="text-center mt-20">
                    <p className="text-slate-400 font-bold uppercase tracking-widest">Không có đơn nghỉ phép nào trong hệ thống</p>
                </div>
            )}
        </div>
    );
