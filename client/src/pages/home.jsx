...
const Home = ({ jobs, loading }) => {
  ...
  return (
    <div className="space-y-16 animate-in fade-in duration-700 pb-20 text-left">

      {/* SECTION 1: HERO - GIỮ NGUYÊN BỐ CỤC ĐẸP CỦA BRO */}
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4 pt-4">
              ...
              <a href={`tel:${myPhoneNumber}`} className="inline-flex items-center gap-4 bg-indigo-800/30 backdrop-blur-md text-white border border-white/20 px-8 py-5 rounded-[2.5rem] font-bold text-lg hover:bg-indigo-800/50 transition-all">
                📞 HOTLINE
              </a>
            </div>
          </div>
            ...
            <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-8 rounded-[3rem] shadow-2xl transform rotate-3 hover:rotate-0 transition-transform duration-500">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-indigo-200 italic font-medium animate-pulse font-sans">Đang trực tuyến...</p>
                </div>
              </div>
              <div className="space-y-3">
                <div className="bg-white/5 p-4 rounded-2xl text-sm border border-white/10 italic hover:bg-white/10 transition-colors">"Tìm cho tôi danh sách lập trình viên ReactJS?"</div>
                <div className="bg-white/5 p-4 rounded-2xl text-sm border border-white/10 italic hover:bg-white/10 transition-colors">"Lương trung bình của phòng kỹ thuật là bao nhiêu?"</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="px-4">
        ...
        {loading ? (
          <div className="py-32 text-center">
            ...
            <p className="text-slate-400 font-bold tracking-widest uppercase text-sm font-sans">Đang tải dữ liệu...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {jobs?.map(job => (
              <div
                ...
                className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 hover:shadow-2xl hover:shadow-indigo-500/10 transition-all cursor-pointer group hover:-translate-y-2 flex flex-col justify-between min-h-[350px]"
              >
                <div>
                ...
                <div className="pt-6 border-t border-slate-50 flex justify-between items-center">
                  <div className="space-y-1">
                    <span className="block text-[10px] font-black text-slate-400 uppercase tracking-tighter">Mức lương</span>
                    <span className="text-emerald-600 font-black text-xl">{job.salary}</span>
                  </div>
                  <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center group-hover:bg-indigo-600 group-hover:text-white transition-all text-xl">
                    →
                  </div>
                </div>
              </div>
            ...
            <div className="bg-indigo-600 p-10 text-white flex justify-between items-start">
              <div className="text-left">
                ...
                <div className="flex gap-2">
                  <span className="bg-white/20 px-3 py-1 rounded-full text-[10px] font-bold border border-white/30 tracking-widest uppercase">
                    {selectedJob.department_name}
                  </span>
                  <span className="bg-white/20 px-3 py-1 rounded-full text-[10px] font-bold border border-white/30 italic uppercase">NHÓM 21 VERIFIED</span>
                </div>
              </div>
